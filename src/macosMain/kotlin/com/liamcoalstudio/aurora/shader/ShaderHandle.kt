...
import com.liamcoalstudio.aurora.Vector3
import com.liamcoalstudio.aurora.Vector4
...
actual class ShaderHandle<T> internal actual constructor(internal val handle: UInt) : Resource() {
    ...
    actual fun constructDrawCollection(vararg values: T): DrawCollection<T> {
        ...
        for((name, type, f) in inputs) {
            val l = glGetAttribLocation(handle, name)
            val (s, t) = when (type) {
            ...
            glVertexAttribPointer(l.convert(), s, t.convert(), 0u, size, index.toCPointer<CPointed>())
